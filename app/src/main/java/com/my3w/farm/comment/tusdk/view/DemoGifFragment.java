...
public class DemoGifFragment extends TuFragment implements TuGifView.TuGifViewDelegate {
	...
	protected void viewDidLoad(ViewGroup view) {
		 ...
		 * 
		 * // 自动播放 (默认: true) gif.setAutoPlay(true);
		 * 
		 * gif.setImageResource(R.raw.sample_gif_editor); // 侦听播放事件 gif.setDelegate(this); gif.setOnClickListener(mClickListener);
		 * 
		 * TuSdkGifDrawable drawable = (TuSdkGifDrawable)gif.getDrawable();
		 * 
		 * if (drawable != null) { TLog.d("Gif size: {%d, %d}", drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
		 * 
		 * // 设置播放次数 drawable.setLoopCount(3); }
		 * 
		 * RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		 * params.addRule(RelativeLayout.CENTER_IN_PARENT); gif.setLayoutParams(params);
		 * 
		 * view.addView(gif);
		 */
	}
