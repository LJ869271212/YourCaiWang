 ...
 * 
 */
package com.my3w.farm.comment.tusdk.examples.suite;
 ...
 * @author Clear
 */
public class CameraComponentBorrow extends SampleBase implements TuCameraFragmentDelegate {

	private TuCameraFragment fragment;
	...
	/** 相机组件范例 */
	public CameraComponentBorrow(Context context, onComponentListener listener) {
	 ...
	 * 获取一个拍摄结果。
	 * 
	 * 相机的拍摄结果是TuSdkResult对象，依照设置，输出结果可能是 Bitmap、File或者ImageSqlInfo。 在本例中，拍摄结束后直接关闭了相机界面，依照需求，还可以将拍摄结果作为输入源传给编辑组件，从而实现拍摄编辑一体操作。 欢迎访问文档中心
	 * http://tusdk.com/doc 查看更多示例。
	 * 
	 * @param fragment
	 *            默认相机视图控制器
	 * @param result
	...
	public void onTuCameraFragmentCaptured(TuCameraFragment fragment, TuSdkResult result) {
		...
		http.execute(context.getResources().getString(R.string.config_host_url) + "borrow.php?action=upload&&token="
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				try {
					...
					if (result == 0) {
						Log.e("CameraComponentBorrow", jsonObject.toString());
