 ...
 *
 */
public class PlantToolsSowActivity extends baseActivity {

	private String[] toolsList = new String[] { "补苗", "杀虫", "除草", "游戏", "收割" };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_tools_sow);
	...
	private void initView() {
		...
		title_name.setText("播种");

		title_username = (TextView) findViewById(R.id.title_username);
		String user_nickString = DataCache.get(PlantToolsSowActivity.this).getAsString("user_nick");
		...
		String user_picString = DataCache.get(PlantToolsSowActivity.this).getAsString("user_pic");
	...
	private void initData() {
		HttpRequest http = new HttpRequest(PlantToolsSowActivity.this, SelectLandhandler);
		http.execute(getResources().getString(R.string.config_host_url) + "tools.php?action=getland&token="
				+ DataCache.get(PlantToolsSowActivity.this).getAsString("token"));
	}
	...
	private void initEvent() {
		...
		title_back.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				new AlertDialog.Builder(PlantToolsSowActivity.this).setTitle("点击选择操作")
						.setItems(toolsList, new DialogInterface.OnClickListener() {

					public void onClick(DialogInterface dialog, int which) {
						Intent intent = null;
						if (which == 0) {
							intent = new Intent(PlantToolsSowActivity.this, PlantToolsBumiaoActivity.class);
						} else if (which == 1) {
							intent = new Intent(PlantToolsSowActivity.this, PlantToolsInsecticidalActivity.class);
						} else if (which == 2) {
							intent = new Intent(PlantToolsSowActivity.this, PlantToolsWeedActivity.class);
						} else if (which == 3) {
							intent = null;
						} else if (which == 4) {
							intent = new Intent(PlantToolsSowActivity.this, PlantToolsHarvestActivity.class);
						}
		...
		back_button.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(PlantToolsSowActivity.this, PlantActivity.class);
				startActivity(intent);
				finish();
		...
		title_icon_box.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(PlantToolsSowActivity.this, UserCenterActivity.class);
				startActivity(intent);
				finish();
				overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
			}
		...
		select_land.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (LandNoList != null && LandNoList.length > 0) {
					new AlertDialog.Builder(PlantToolsSowActivity.this).setTitle("选择土地编号")
					...
					Toasts.showToast(PlantToolsSowActivity.this, "土地获取中，请稍后……");
				}
			}
		});
		...
		select_seed.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (SeedsNameList != null && SeedsNameList.length > 0) {
					new AlertDialog.Builder(PlantToolsSowActivity.this).setTitle("选择种子")
					...
					Toasts.showToast(PlantToolsSowActivity.this, "种子获取中，请稍后……");
				}
			}
		});
		...
		select_unit.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				new AlertDialog.Builder(PlantToolsSowActivity.this).setTitle("选择单位")
		...
		seeds_date.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new DatePickerDialog(PlantToolsSowActivity.this, new OnDateSetListener() {
		...
		submit_button.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (IndexLandId == null || "".equals(IndexLandId)) {
					Toast.makeText(PlantToolsSowActivity.this, "请选择土地", Toast.LENGTH_SHORT).show();
				} else if (IndexSeedsId == null || "".equals(IndexSeedsId)) {
					Toast.makeText(PlantToolsSowActivity.this, "请选择种子", Toast.LENGTH_SHORT).show();
				} else if ("".equals(seeds_number.getText().toString())) {
					Toast.makeText(PlantToolsSowActivity.this, "请输入种植数量", Toast.LENGTH_SHORT).show();
				} else if ("".equals(seeds_date.getText().toString())) {
					Toast.makeText(PlantToolsSowActivity.this, "请选择种植日期", Toast.LENGTH_SHORT).show();
				} else {
					...
					HttpRequest http = new HttpRequest(PlantToolsSowActivity.this, headerList, Submithandler);
					http.execute(getResources().getString(R.string.config_host_url) + "tools.php?action=sow&token="
							+ DataCache.get(PlantToolsSowActivity.this).getAsString("token"));
				}
			}
		});
	...
	private Handler SelectLandhandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				try {
					...
					if (result == 0) {
						...
						if (jsonArray.length() > 0) {
							...
							Toast.makeText(PlantToolsSowActivity.this, "还没有土地，快去买一块吧", Toast.LENGTH_SHORT).show();
							Intent intent = new Intent(PlantToolsSowActivity.this, ShopLandActivity.class);
							startActivity(intent);
							finish();
							overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
						}
					} else if (result == 1) {
						Toast.makeText(PlantToolsSowActivity.this, "登录超时或已经被别人登录，请重新登录", Toast.LENGTH_SHORT).show();
						Intent intent = new Intent(PlantToolsSowActivity.this, LoginActivity.class);
						startActivity(intent);
				...
				Toast.makeText(PlantToolsSowActivity.this, "网络出现了点问题，请查看网络连接是否正常后再重试", Toast.LENGTH_SHORT).show();
			}
		};
	};
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				try {
					...
					int result = jsonObject.optInt("errCode");
					if (result == 0) {
						Toast.makeText(PlantToolsSowActivity.this, "播种申请成功，请等待工人施工", Toast.LENGTH_SHORT).show();
						Intent intent = new Intent(PlantToolsSowActivity.this, PlantActivity.class);
						startActivity(intent);
						finish();
						overridePendingTransition(R.anim.anim_activity_close_left, R.anim.anim_activity_close_right);
	...
	private void getSeeds(String id) {
		HttpRequest http = new HttpRequest(PlantToolsSowActivity.this, SelectSeedshandler);
		http.execute(getResources().getString(R.string.config_host_url) + "tools.php?action=getseed&token="
				+ DataCache.get(PlantToolsSowActivity.this).getAsString("token") + "&goods_id=" + id);
			...
			if (msg.what == 200) {
				...
				try {
					...
					if (result == 0) {
						...
						Toast.makeText(PlantToolsSowActivity.this, "还没有种子呢，快去买点吧", Toast.LENGTH_SHORT).show();
						Intent intent = new Intent(PlantToolsSowActivity.this, ShopSeedActivity.class);
						startActivity(intent);
						finish();
						overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
					}
