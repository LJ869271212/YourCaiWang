 ...
 *
 */
public class BorrowHuanCaiXuQiuInfoActivity extends baseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_game_1_borrow_info_4);
	...
	private void initView() {
		...
		String user_picString = DataCache.get(BorrowHuanCaiXuQiuInfoActivity.this).getAsString("user_pic");
		...
		String user_nickString = DataCache.get(BorrowHuanCaiXuQiuInfoActivity.this).getAsString("user_nick");
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				if (json != null) {
					...
					try {
						...
						if (result == 0) {
							...
							Toast.makeText(BorrowHuanCaiXuQiuInfoActivity.this, "登录超时或已经被别人登录，请重新登录", Toast.LENGTH_SHORT).show();
							Intent intent = new Intent(BorrowHuanCaiXuQiuInfoActivity.this, LoginActivity.class);
							startActivity(intent);
							sendBroadcast(new Intent("finish"));
							overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
						}
					} catch (JSONException e) {
						Toast.makeText(BorrowHuanCaiXuQiuInfoActivity.this, "数据出现异常，请重试", Toast.LENGTH_SHORT).show();
				...
				Toast.makeText(BorrowHuanCaiXuQiuInfoActivity.this, "网络出现了点问题，请查看网络连接是否正常后再重试", Toast.LENGTH_SHORT).show();
