 ...
 *
 */
public class BorrowJieCaiXuQiuAddActivity extends baseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_game_1_borrow_add_3);
	...
	private void initView() {
		...
		String user_picString = DataCache.get(BorrowJieCaiXuQiuAddActivity.this).getAsString("user_pic");
		...
		String user_nickString = DataCache.get(BorrowJieCaiXuQiuAddActivity.this).getAsString("user_nick");
	...
	private void initData() {
		HttpRequest http = new HttpRequest(BorrowJieCaiXuQiuAddActivity.this, SelectLandhandler);
		http.execute(getResources().getString(R.string.config_host_url) + "tools.php?action=getland&token="
				+ DataCache.get(BorrowJieCaiXuQiuAddActivity.this).getAsString("token"));
	}
	...
	private void initEvent() {
		...
		land_name.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (LandNoList != null && LandNoList.length > 0) {
					new AlertDialog.Builder(BorrowJieCaiXuQiuAddActivity.this).setTitle("选择土地编号")
					...
					Toasts.showToast(BorrowJieCaiXuQiuAddActivity.this, "土地获取中，请稍后……");
				}
			}
		});
		...
		borrow_unit.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				new AlertDialog.Builder(BorrowJieCaiXuQiuAddActivity.this).setTitle("选择单位")
		...
		re_date1.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new DatePickerDialog(BorrowJieCaiXuQiuAddActivity.this, new OnDateSetListener() {
		...
		upload.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new AlertDialog.Builder(BorrowJieCaiXuQiuAddActivity.this).setTitle("选择图片上传方式")
						.setItems(addList, new DialogInterface.OnClickListener() {

					public void onClick(DialogInterface dialog, int which) {

						if (which == 0) { // 相册选择
							new AlbumComponentBorrow(BorrowJieCaiXuQiuAddActivity.this, new AlbumComponentBorrow.onComponentListener() {

								@Override
								public void error(int val) {
									if (val == -1) {
										Toast.makeText(BorrowJieCaiXuQiuAddActivity.this, "登录超时或已经被别人登录，请重新登录", Toast.LENGTH_SHORT).show();
										Intent intent = new Intent(BorrowJieCaiXuQiuAddActivity.this, LoginActivity.class);
										startActivity(intent);
										sendBroadcast(new Intent("finish"));
										overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
									} else {
										Toast.makeText(BorrowJieCaiXuQiuAddActivity.this, "相册出了点小问题，请重试", Toast.LENGTH_LONG).show();
									}
								}

								@Override
								public void component(String path, String file) {
									Toast.makeText(BorrowJieCaiXuQiuAddActivity.this, "图片上传成功", Toast.LENGTH_LONG).show();
									BorrowJieCaiXuQiuAddActivity.this.file = path + file;
								}
							}).showSample(BorrowJieCaiXuQiuAddActivity.this);
						} else if (which == 1) { // 拍照
							new CameraComponentBorrow(BorrowJieCaiXuQiuAddActivity.this, new CameraComponentBorrow.onComponentListener() {
								...
								public void error(int val) {
									if (val == -1) {
										...
										Toast.makeText(BorrowJieCaiXuQiuAddActivity.this, "拍照除了点小问题，请重试", Toast.LENGTH_LONG).show();
									}
								}
		...
		submit_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (checkPost(sfile, "封面图不存在，请上传图片"))
					...
					return;
				else if (checkPost(sdescription, "还菜描述不能为空"))
					return;
					...
					HttpRequest httpseed = new HttpRequest(BorrowJieCaiXuQiuAddActivity.this, headerList, SubmitHeandler);
					httpseed.execute(BorrowJieCaiXuQiuAddActivity.this.getResources().getString(R.string.config_host_url)
							+ "borrow.php?action=add&token=" + DataCache.get(BorrowJieCaiXuQiuAddActivity.this).getAsString("token"));
				}
			}
		});
	...
	private Handler SelectLandhandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				try {
					...
					if (result == 0) {
						...
						if (jsonArray.length() > 0) {
							...
							Toast.makeText(BorrowJieCaiXuQiuAddActivity.this, "还没有土地，快去买一块吧", Toast.LENGTH_SHORT).show();
							Intent intent = new Intent(BorrowJieCaiXuQiuAddActivity.this, ShopLandActivity.class);
							startActivity(intent);
							finish();
							overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
						}
				...
				Toast.makeText(BorrowJieCaiXuQiuAddActivity.this, "网络出现了点问题，请查看网络连接是否正常后再重试", Toast.LENGTH_SHORT).show();
			}
		};
	};
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				try {
					...
					int result = jsonObject.optInt("errCode");
					if (result == 0) {
						Toast.makeText(BorrowJieCaiXuQiuAddActivity.this, "借菜发布成功", Toast.LENGTH_SHORT).show();
						finish();
