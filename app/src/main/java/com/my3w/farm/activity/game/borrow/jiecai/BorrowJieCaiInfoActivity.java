 ...
 *
 */
public class BorrowJieCaiInfoActivity extends baseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_game_1_borrow_info_1);
	...
	private void initView() {
		...
		title_name.setText("借菜详情");
		title_icon = (RoundImageView) this.findViewById(R.id.title_icon);
		String user_picString = DataCache.get(BorrowJieCaiInfoActivity.this).getAsString("user_pic");
		...
		String user_nickString = DataCache.get(BorrowJieCaiInfoActivity.this).getAsString("user_nick");
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				if (json != null) {
					...
					try {
						...
						if (result == 0) {
							...
							Toast.makeText(BorrowJieCaiInfoActivity.this, "登录超时或已经被别人登录，请重新登录", Toast.LENGTH_SHORT).show();
							Intent intent = new Intent(BorrowJieCaiInfoActivity.this, LoginActivity.class);
							startActivity(intent);
							sendBroadcast(new Intent("finish"));
							overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
						}
					} catch (JSONException e) {
						Toast.makeText(BorrowJieCaiInfoActivity.this, "数据出现异常，请重试", Toast.LENGTH_SHORT).show();
				...
				Toast.makeText(BorrowJieCaiInfoActivity.this, "网络出现了点问题，请查看网络连接是否正常后再重试", Toast.LENGTH_SHORT).show();
