package com.my3w.farm.activity.game.borrow.huancai;
 ...
 *
 */
public class BorrowHuanCaiAddActivity extends baseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_game_1_borrow_add_2);
	...
	private void initView() {
		...
		title_name.setText("发布换菜");
		title_icon = (RoundImageView) this.findViewById(R.id.title_icon);
		String user_picString = DataCache.get(BorrowHuanCaiAddActivity.this).getAsString("user_pic");
		...
		String user_nickString = DataCache.get(BorrowHuanCaiAddActivity.this).getAsString("user_nick");
	...
	private void initData() {
		HttpRequest http = new HttpRequest(BorrowHuanCaiAddActivity.this, SelectLandhandler);
		http.execute(getResources().getString(R.string.config_host_url) + "tools.php?action=getland&token="
				+ DataCache.get(BorrowHuanCaiAddActivity.this).getAsString("token"));
	}
	...
	private void initEvent() {
		...
		land_name.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (LandNoList != null && LandNoList.length > 0) {
					new AlertDialog.Builder(BorrowHuanCaiAddActivity.this).setTitle("选择土地编号")
					...
					Toasts.showToast(BorrowHuanCaiAddActivity.this, "土地获取中，请稍后……");
				}
			}
		});
		...
		borrow_name.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (SeedList != null && SeedList.length > 0) {
					new AlertDialog.Builder(BorrowHuanCaiAddActivity.this).setTitle("点击选择菜名")
					...
					Toast.makeText(BorrowHuanCaiAddActivity.this, "土地还没有种任何菜", Toast.LENGTH_SHORT).show();
				}
			}
		});
		...
		borrow_unit.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				new AlertDialog.Builder(BorrowHuanCaiAddActivity.this).setTitle("选择单位")
		...
		br_date1.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new DatePickerDialog(BorrowHuanCaiAddActivity.this, new OnDateSetListener() {
		...
		upload.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new AlertDialog.Builder(BorrowHuanCaiAddActivity.this).setTitle("选择图片上传方式")
						.setItems(addList, new DialogInterface.OnClickListener() {

					public void onClick(DialogInterface dialog, int which) {

						if (which == 0) { // 相册选择
							new AlbumComponentBorrow(BorrowHuanCaiAddActivity.this, new AlbumComponentBorrow.onComponentListener() {

								@Override
								public void error(int val) {
									if (val == -1) {
										Toast.makeText(BorrowHuanCaiAddActivity.this, "登录超时或已经被别人登录，请重新登录", Toast.LENGTH_SHORT).show();
										Intent intent = new Intent(BorrowHuanCaiAddActivity.this, LoginActivity.class);
										startActivity(intent);
										sendBroadcast(new Intent("finish"));
										overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
									} else {
										Toast.makeText(BorrowHuanCaiAddActivity.this, "相册出了点小问题，请重试", Toast.LENGTH_LONG).show();
									}
								}

								@Override
								public void component(String path, String file) {
									Toast.makeText(BorrowHuanCaiAddActivity.this, "图片上传成功", Toast.LENGTH_LONG).show();
									BorrowHuanCaiAddActivity.this.file = path + file;
								}
							}).showSample(BorrowHuanCaiAddActivity.this);
						} else if (which == 1) { // 拍照
							new CameraComponentBorrow(BorrowHuanCaiAddActivity.this, new CameraComponentBorrow.onComponentListener() {
								...
								public void error(int val) {
									if (val == -1) {
										...
										Toast.makeText(BorrowHuanCaiAddActivity.this, "拍照除了点小问题，请重试", Toast.LENGTH_LONG).show();
									}
								}
		...
		submit_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (checkPost(sfile, "封面图不存在，请上传图片"))
					...
					return;
				else if (checkPost(sbr_date1, "预定换菜日期不能为空"))
					return;
				else {
					...
					HttpRequest httpseed = new HttpRequest(BorrowHuanCaiAddActivity.this, headerList, SubmitHeandler);
					httpseed.execute(BorrowHuanCaiAddActivity.this.getResources().getString(R.string.config_host_url)
							+ "borrow.php?action=add&token=" + DataCache.get(BorrowHuanCaiAddActivity.this).getAsString("token"));
				}
			}
		});
	...
	private Handler SelectLandhandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				try {
					...
					if (result == 0) {
						...
						if (jsonArray.length() > 0) {
							...
							Toast.makeText(BorrowHuanCaiAddActivity.this, "还没有土地，快去买一块吧", Toast.LENGTH_SHORT).show();
							Intent intent = new Intent(BorrowHuanCaiAddActivity.this, ShopLandActivity.class);
							startActivity(intent);
							finish();
							overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
						}
				...
				Toast.makeText(BorrowHuanCaiAddActivity.this, "网络出现了点问题，请查看网络连接是否正常后再重试", Toast.LENGTH_SHORT).show();
			}
		};
	};
	...
	private void getSends(String keyon) {
		...
		HttpRequest httpseed = new HttpRequest(BorrowHuanCaiAddActivity.this, seedApiHandler);
		httpseed.execute(
				BorrowHuanCaiAddActivity.this.getResources().getString(R.string.config_host_url) + "control.php?action=getseed&token="
						+ DataCache.get(BorrowHuanCaiAddActivity.this).getAsString("token") + "&keynum=" + keyon);
	}
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				try {
					...
					int result = jsonObject.optInt("errCode");
					if (result == 0) {
						Toast.makeText(BorrowHuanCaiAddActivity.this, "借菜发布成功", Toast.LENGTH_SHORT).show();
						finish();
