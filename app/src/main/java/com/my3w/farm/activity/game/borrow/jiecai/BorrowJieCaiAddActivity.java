package com.my3w.farm.activity.game.borrow.jiecai;
 ...
 *
 */
public class BorrowJieCaiAddActivity extends baseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_game_1_borrow_add_1);
	...
	private void initView() {
		...
		title_name.setText("发布借菜");
		title_icon = (RoundImageView) this.findViewById(R.id.title_icon);
		String user_picString = DataCache.get(BorrowJieCaiAddActivity.this).getAsString("user_pic");
		...
		String user_nickString = DataCache.get(BorrowJieCaiAddActivity.this).getAsString("user_nick");
		...
		borrow_name = (TextView) this.findViewById(R.id.borrow_name);
		...
		re_date1 = (TextView) this.findViewById(R.id.re_date1);
	...
	private void initData() {
		HttpRequest http = new HttpRequest(BorrowJieCaiAddActivity.this, SelectLandhandler);
		http.execute(getResources().getString(R.string.config_host_url) + "tools.php?action=getland&token="
				+ DataCache.get(BorrowJieCaiAddActivity.this).getAsString("token"));
	}
	...
	private void initEvent() {
		...
		land_name.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (LandNoList != null && LandNoList.length > 0) {
					new AlertDialog.Builder(BorrowJieCaiAddActivity.this).setTitle("选择土地编号")
							.setItems(LandNoList, new DialogInterface.OnClickListener() {

						public void onClick(DialogInterface dialog, int which) {
							land_name.setText(LandNoList[which]);

							getSends(LandNoList[which]);
						}
					}).show();
				} else {
					Toasts.showToast(BorrowJieCaiAddActivity.this, "土地获取中，请稍后……");
				}
			}
		});

		// 选择种子
		borrow_name.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				if (SeedList != null && SeedList.length > 0) {
					new AlertDialog.Builder(BorrowJieCaiAddActivity.this).setTitle("点击选择菜名")
							.setItems(SeedList, new DialogInterface.OnClickListener() {

						public void onClick(DialogInterface dialog, int which) {
							borrow_name.setText(SeedList[which]);
						}
					}).show();
				} else {
					Toast.makeText(BorrowJieCaiAddActivity.this, "土地还没有种任何菜", Toast.LENGTH_SHORT).show();
				}
			}
		});
		...
		borrow_unit.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				new AlertDialog.Builder(BorrowJieCaiAddActivity.this).setTitle("选择单位")
		...
		// 选择日期
		re_date1.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				int month = 0;
				int day = 0;
				if (re_date1.getText().toString().equals("")) {
					...
					String getDate = re_date1.getText().toString();
				...
				new DatePickerDialog(BorrowJieCaiAddActivity.this, new OnDateSetListener() {

					@Override
					public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
						re_date1.setText(String.valueOf(year) + "-" + String.valueOf(monthOfYear + 1) + "-" + String.valueOf(dayOfMonth));
					}
				}, year, month, day).show();
			}
		});
		...
		upload.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new AlertDialog.Builder(BorrowJieCaiAddActivity.this).setTitle("选择图片上传方式")
						.setItems(addList, new DialogInterface.OnClickListener() {

					public void onClick(DialogInterface dialog, int which) {

						if (which == 0) { // 相册选择
							new AlbumComponentBorrow(BorrowJieCaiAddActivity.this, new AlbumComponentBorrow.onComponentListener() {

								@Override
								public void error(int val) {
									if (val == -1) {
										Toast.makeText(BorrowJieCaiAddActivity.this, "登录超时或已经被别人登录，请重新登录", Toast.LENGTH_SHORT).show();
										Intent intent = new Intent(BorrowJieCaiAddActivity.this, LoginActivity.class);
										startActivity(intent);
										sendBroadcast(new Intent("finish"));
										overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
									} else {
										Toast.makeText(BorrowJieCaiAddActivity.this, "相册出了点小问题，请重试", Toast.LENGTH_LONG).show();
									}
								}

								@Override
								public void component(String path, String file) {
									Toast.makeText(BorrowJieCaiAddActivity.this, "图片上传成功", Toast.LENGTH_LONG).show();
									BorrowJieCaiAddActivity.this.file = path + file;
								}
							}).showSample(BorrowJieCaiAddActivity.this);
						} else if (which == 1) { // 拍照
							new CameraComponentBorrow(BorrowJieCaiAddActivity.this, new CameraComponentBorrow.onComponentListener() {
								...
								public void error(int val) {
									if (val == -1) {
										...
										Toast.makeText(BorrowJieCaiAddActivity.this, "拍照除了点小问题，请重试", Toast.LENGTH_LONG).show();
									}
								}
		...
		submit_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String sre_date1 = re_date1.getText().toString();
				...
				if (checkPost(sfile, "封面图不存在，请上传图片"))
					...
					return;
				else if (checkPost(sbr_date1, "借菜日期不能为空"))
					return;
				else if (checkPost(sreturn_name, "还菜名称不能为空"))
					return;
					...
					headerList.put("re_date", sre_date1);
					headerList.put("files", sfile);
					headerList.put("borrow_unit", sborrow_unit);
					headerList.put("borrow_price", sborrow_price);
					HttpRequest httpseed = new HttpRequest(BorrowJieCaiAddActivity.this, headerList, SubmitHeandler);
					httpseed.execute(BorrowJieCaiAddActivity.this.getResources().getString(R.string.config_host_url)
							+ "borrow.php?action=add&token=" + DataCache.get(BorrowJieCaiAddActivity.this).getAsString("token"));
				}
			}
		});
	...
	private Handler SelectLandhandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				try {
					...
					if (result == 0) {
						...
						if (jsonArray.length() > 0) {
							...
							Toast.makeText(BorrowJieCaiAddActivity.this, "还没有土地，快去买一块吧", Toast.LENGTH_SHORT).show();
							Intent intent = new Intent(BorrowJieCaiAddActivity.this, ShopLandActivity.class);
							startActivity(intent);
							finish();
							overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
						}
				...
				Toast.makeText(BorrowJieCaiAddActivity.this, "网络出现了点问题，请查看网络连接是否正常后再重试", Toast.LENGTH_SHORT).show();
			}
		};
	};
	...
	private void getSends(String keyon) {
		...
		HttpRequest httpseed = new HttpRequest(BorrowJieCaiAddActivity.this, seedApiHandler);
		httpseed.execute(
				BorrowJieCaiAddActivity.this.getResources().getString(R.string.config_host_url) + "control.php?action=getseed&token="
						+ DataCache.get(BorrowJieCaiAddActivity.this).getAsString("token") + "&keynum=" + keyon);
	}
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				try {
					...
					int result = jsonObject.optInt("errCode");
					if (result == 0) {
						Toast.makeText(BorrowJieCaiAddActivity.this, "借菜发布成功", Toast.LENGTH_SHORT).show();
						finish();
