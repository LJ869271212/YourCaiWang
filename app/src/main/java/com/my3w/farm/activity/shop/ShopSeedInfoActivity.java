...
public class ShopSeedInfoActivity extends baseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_shop_seed_info);
	...
	private void initView() {
		...
		String user_picString = DataCache.get(ShopSeedInfoActivity.this).getAsString("user_pic");
	...
	private void initData() {
		...
		http.execute(getResources().getString(R.string.config_host_url) + "seedshop.php?action=info&token="
	...
	private void initEvent() {
		...
		title_icon_box.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(ShopSeedInfoActivity.this, UserCenterActivity.class);
		...
		shop_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// 跳转购物车
				Intent intent = new Intent(ShopSeedInfoActivity.this, ShopCartActivity.class);
		...
		add_btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Toast.makeText(ShopSeedInfoActivity.this, "添加购物车成功", Toast.LENGTH_SHORT).show();
	...
	private void addCart() {
		...
		sqlite.open(ShopSeedInfoActivity.this);
		...
		contentValues.put("wheres", "seeds");
		contentValues.put("level", DataCache.get(ShopSeedInfoActivity.this).getAsString("user_level"));

		try {
			Cursor cursors = sqlite.findById("SeedCart", "id", Integer.parseInt(id), null);
			if (cursors != null) {
				if (cursors.getCount() <= 0) {
					sqlite.insert("SeedCart", null, contentValues);
				} else {
					...
					sqlite.udpate("SeedCart", new String[] { "id" }, new String[] { id }, contentValuesUpdate);
				}
				cursors.close();
			} else {
				sqlite.insert("SeedCart", null, contentValues);
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 200) {
				...
				if (json != null) {
					...
					try {
						...
						if (result == 0) {
							...
							info_type.setText(ctype);
							String date = jsonObject.optString("grow_day");
							info_date.setText(date + "天");
							...
							ShopSeedInfoActivity.this.content = ctype + "生长周期" + date + "天";
							ShopSeedInfoActivity.this.title = title;
							ShopSeedInfoActivity.this.price = price;
							ShopSeedInfoActivity.this.pic = thumb;
							ShopSeedInfoActivity.this.unit = unit;
							ShopSeedInfoActivity.this.use_unit = use_unit;
							ShopSeedInfoActivity.this.use_number = use_number;
						} else if (result == 1) {
							Toast.makeText(ShopSeedInfoActivity.this, "登录超时或已经被别人登录，请重新登录", Toast.LENGTH_SHORT).show();
							Intent intent = new Intent(ShopSeedInfoActivity.this, LoginActivity.class);
							startActivity(intent);
							sendBroadcast(new Intent("finish"));
							overridePendingTransition(R.anim.anim_activity_open_left, R.anim.anim_activity_open_right);
						}
					} catch (JSONException e) {
						Toast.makeText(ShopSeedInfoActivity.this, "数据出现异常，请重试", Toast.LENGTH_SHORT).show();
				...
				Toast.makeText(ShopSeedInfoActivity.this, "网络出现了点问题，请查看网络连接是否正常后再重试", Toast.LENGTH_SHORT).show();
