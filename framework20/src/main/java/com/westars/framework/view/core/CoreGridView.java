...
/**
 * A {@link GridView} that supports adding header rows in a very similar way to {@link android.widget.ListView}. See
 ...
 * 
 * @author https
 *         ://github.com/liaohuqiu/android-GridViewWithHeaderAndFooter/blob/master/src/in/srain/cube/views/GridViewWithHeaderAndFooter.java
 * 
 */
public class CoreGridView extends GridView {
	...
	public CoreGridView(Context context) {
		super(context);
		initHeaderGridView();
	}

	public CoreGridView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initHeaderGridView();
	}

	public CoreGridView(Context context, AttributeSet attrs, int defStyle) {
	...
	/**
	 * Add a fixed view to appear at the top of the grid. If addHeaderView is called more than once, the views will appear in the order they
	 * were added. Views added using this call can take focus if they want.
	 * <p/>
	 * NOTE: Call this before calling setAdapter. This is so HeaderGridView can wrap the supplied cursor with one that will also account for
	 * header views.
	 * 
	 * @param v
	 *            The view to add.
	...
	private class FullWidthFixedViewLayout extends FrameLayout {
		...
		protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
			int realLeft = CoreGridView.this.getPaddingLeft() + getPaddingLeft();
		...
		@Override
		protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
			int targetWidth = CoreGridView.this.getMeasuredWidth() - CoreGridView.this.getPaddingLeft()
					- CoreGridView.this.getPaddingRight();
	...
	/**
	 * ListAdapter used when a HeaderGridView has header views. This ListAdapter wraps another one and also keeps track of the header views
	...
	public static class HeaderViewGridAdapter implements WrapperListAdapter, Filterable {
		...
		/**
		 * @return true if this adapter doesn't contain any data. This is used to determine whether the empty view should be displayed. A
		 *         typical implementation will return getCount() == 0 but since getCount() includes the headers and footers, specialized
		...
		public int getItemViewType(int position) {
			...
			if (DEBUG) {
				Log.d(LOG_TAG,
						String.format("getItemViewType: pos: %s, result: %s", position, type, mCachePlaceHoldView, mCacheFirstHeaderView));
			}
			return type;
		}
