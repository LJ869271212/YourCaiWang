...
/**
 * WestarsListView下载刷新，上拉加载
 * 
 * @author Aports
 * 
 */
public class ListExtView extends ListView implements OnScrollListener {
	...
	public ListExtView(Context context) {
		super(context);
		init(context);
	}

	public ListExtView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public ListExtView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
		...
		if (showFooterView) {
			if (loadNumber != 0) {
				int onfirstVisibleItem = (firstVisibleItem + 1) - getHeaderViewsCount();
				int ontotalItemCount = totalItemCount - getHeaderViewsCount();
				if (ontotalItemCount - onfirstVisibleItem == loadNumber) {
					if (FOOTERVIEW_STATE == FOOTERVIEW_NONE) {
						if (listener != null) {
							FOOTERVIEW_STATE = FOOTERVIEW_REFRESHING;
							listener.Load();
